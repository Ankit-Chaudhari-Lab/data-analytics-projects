# Churn Dataset Analysis Dashboard

## Project Overview
This project analyses customer churn data to identify high-risk customer segments and understand key factors influencing churn. Using Python for data cleaning and exploratory analysis, and Power BI for interactive dashboard creation, the project helps visualize trends in service usage, tenure, billing, and customer details, enabling better retention strategies.

## Tools Used
- **Power BI** – Interactive dashboards and KPI tracking  
- **Python** (Pandas, NumPy, Matplotlib, Seaborn) – Data cleaning and visualization  
- **SQL / Excel** – Data querying and preparation

## Key Insights
- Identified high-risk customer segments likely to churn  
- Key churn patterns observed in service usage, billing, and tenure  
- Insights support targeted retention strategies for 1000+ customers

## Dashboard Screenshot
![Churn Dashboard](01Churn.png)
![Churn Dashboard](02Churn.png)
![Churn Dashboard](03Churn.png)

## Dataset
- 7,000+ rows of anonymized customer data  
- Columns include: Customer ID, Tenure, Service Usage, Billing Details, Churn Status  
- Dataset source: [mention source if allowed]

## How to Run
1. Open the Power BI file (`.pbix`) to explore interactive dashboards  
2. Open Python notebook (`.ipynb`) to view preprocessing, EDA, and analysis code  
3. Install required Python packages: `pandas`, `numpy`, `matplotlib`, `seaborn`
